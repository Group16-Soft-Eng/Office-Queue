...
const ServicePage: React.FC = () => {
  ...
  const serveNextClient = () => {
    if (queue.length > 0) {
      ...
      const servedTicket = updatedQueue.shift(); // Take first customer from queue
      setQueue(updatedQueue);
      setCurrentTicket(servedTicket || null);
      
      // Save changes to browser storage
      localStorage.setItem(`service_${serviceId}_queue`, JSON.stringify(updatedQueue));
      if (servedTicket) {
        localStorage.setItem(`current_serving_${serviceId}`, servedTicket);
      }
    }
  };
